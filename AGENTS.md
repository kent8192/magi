...
- `AGENTS.md` (Codex) and `CLAUDE.md` (Claude Code) are deliberate mirror copies
  kept in sync.
- The two files MUST differ only on this small set of mechanical substitutions:
  - `AGENTS.md` <-> `CLAUDE.md` in titles and references
  - `AGENTS.local.md` <-> `CLAUDE.local.md`
  - `Codex` <-> `Claude Code` when describing agent-specific attribution or
    local preferences
...
@AGENTS.local.md - Project-specific local preferences, if present.

**Note**: This AGENTS.md focuses on core project rules and quick reference.
